...
export const getCharactersByName = async (name = ""): Promise<characterResult> => {
    return fetch(`http://192.168.2.63:8000/characters/?CharacterName=${name}`).then(
        (response) => response.json()
    )
}

// export const getCharactersByName = async (name = ''): Promise<characterResult> => {
//     const response = await fetch(`http://192.168.2.63:8000/api/characters/?CharacterName=${name}`);
...
export const getCharacterById = async (
    ...
    try {
        const response = await fetch(`http://192.168.2.63:8000/characters/${character_id}/`);
        if (!response.ok) {
            const text = await response.text();
